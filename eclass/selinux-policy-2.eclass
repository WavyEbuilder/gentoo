...
selinux-policy-2_src_prepare() {
	...
	mkdir 3rd_party || die "Failed to create directory ${S}/refpolicy/modules/3rd_party"
...
selinux-policy-2_pkg_postinst() {
	...
	if [[ -z ${ROOT} ]]; then
		...
		if [[ -x "${ESYSROOT}/usr/bin/qdepends" ]]; then
			out=$("${ESYSROOT}/usr/bin/qdepends" -CiqqrF '%[CATEGORY]%[PN]%[SLOT]' -Q "${CATEGORY}/${PN}")
			status=$?
			cmd='qdepends'
		elif [[ -x "${ESYSROOT}/usr/bin/equery" ]]; then
			out=$("${ESYSROOT}/usr/bin/equery" -Cq depends "${CATEGORY}/${PN}")
			status=$?
			cmd='equery'
		else
